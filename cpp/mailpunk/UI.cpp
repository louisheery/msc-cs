...
void UI::refreshMailList() {
	...
	// Cycle through all messages in the inbox
	for(size_t i = 0; messages[i]; i++) {

		// Add each message to the list of emails in the inbox view
		(*(viewToMessageMap))[*mailListView->insert(
				{messages[i]->getField("From"), messages[i]->getField("Subject")})] = messages[i];
	
	}

	// If email is clicked -> Display email window
...
// Once User Clicks 'Login' button -> tries to login to email server & Refresh mail list
void UI::loginClicked(FWidget*) {
	...
	// If delete key is pressed on email -> execute email item deletion function
