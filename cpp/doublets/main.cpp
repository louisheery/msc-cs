...
int main()
{
  ...
  const char *repeat_chain[] = {"TEA", "SEA", "TEA", "SEA"};

  valid = valid_chain(repeat_chain);
  cout << "The chain 'TEA->SEA->TEA->SEA' is " << (valid ? "" : "NOT ") << "a valid chain." << endl
       << endl;

  const char *repeat_chain2[] = { "WHEAT", "CHEAP", "WHEAT", "CHEAP", NULL };

  valid = valid_chain(repeat_chain2);
